...
# إعدادات الموقع
base_url = 'https://kitchen.sayidaty.net'
recipe_url_prefix = 'https://kitchen.sayidaty.net/node/'
output_folder = r'C:\Users\ramon\Desktop\SayidatyLinks'
...
def extract_links(url):
    """جلب الروابط من صفحة معينة التي تحتوي على 'recipe_url_prefix'."""
    links = set()
    try:
        ...
        if response.status_code == 200:
            ...
            for link in soup.find_all('a', href=True):
                ...
                if href and href.startswith(recipe_url_prefix) and full_url not in visited_links:
...
input_file = os.path.join(output_folder, 'recipes.txt')
...
# حفظ الروابط الجديدة في الملف النصي
output_file = os.path.join(output_folder, 'recipes.txt')
